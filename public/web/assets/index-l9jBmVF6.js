...
`,w=o=>{const a=e.user?.id===o.userId;return`
...
`,j=()=>{const o=e.user?`<div class="session ok">${t("signedAs")} <b>${u(e.user.username)}</b> (${u(e.user.role)})</div>`:`<div class="session">${t("notAuthorized")}</div>`;return`
  <main class="layout ${e.theme}">
    ...
    <section class="panel ${e.activeTab==="home"?"":"hidden"}">${F()}</section>
    <section class="panel ${e.activeTab==="lobbies"?"":"hidden"}">${H()}</section>
    <section class="panel ${e.activeTab==="profile"?"":"hidden"}">${_()}</section>
    ${e.user?.role==="admin"?`<section class="panel ${e.activeTab==="control"?"":"hidden"}">${x()}</section>`:""}

    ${N()}
  </main>`},g=document.querySelector("#app"),f=async()=>{if(e.token)try{e.user=await l("/auth/me")}catch{e.user=null,e.token="",localStorage.removeItem("stories_token")}},p=()=>{g&&(g.innerHTML=j(),C(p),e.activeTab==="home"&&R(p),e.activeTab==="lobbies"&&L(p),e.activeTab==="profile"&&I(p),e.activeTab==="control"&&e.user?.role==="admin"&&(T(),A()),e.authOpen&&!e.user&&E(p,f))},S=async(o="public",a=4)=>{try{const s=await l(`/lobbies?visibility=${encodeURIComponent(o)}&limit=${a}`);o==="public"&&(e.homeLobbies=s.items||[]),o==="all"&&(e.lobbyCatalog=s.items||[])}catch{o==="public"&&(e.homeLobbies=[]),o==="all"&&(e.lobbyCatalog=[])}};await f();await S("public",4);await S("all",e.lobbyFilters.limit);p();
//# sourceMappingURL=index-l9jBmVF6.js.map
