...
`,f=a=>{const o=t.user?.id===a.userId;return`
  ...
  `},I=()=>{if(!t.activeRoom)return"";const a=Array.isArray(t.activeRoom.participants)?t.activeRoom.participants:[],o=a.filter(c=>c.role!=="spectator"),r=a.filter(c=>c.role==="spectator");return`
    <article class="room-panel">
      ...
      <div class="room-lists">
        <div>
          ...
          <ul class="participant-list">${o.map(f).join("")}</ul>
        </div>
        <div>
          <h4>${e("roomSpectators")} (${r.length})</h4>
          <ul class="participant-list">${r.map(f).join("")}</ul>
        </div>
      </div>
      <div class="stack">
        <h4>${e("roomActions")}</h4>
  ...
  `:`<h2>${e("profileTitle")}</h2><div class="status">${e("needAuthProfile")}</div>`,U=()=>`
  <article>
    <h3>${e("deckLoad")}</h3>
    <div class="stack">
      <select id="adminDeck">
...
`,x=()=>{const a=t.user?`<div class="session ok">${e("signedAs")} <b>${m(t.user.username)}</b> (${m(t.user.role)})</div>`:`<div class="session">${e("notAuthorized")}</div>`;return`
  <main class="layout ${t.theme}">
    ...
    <section class="panel ${t.activeTab==="home"?"":"hidden"}">${O()}</section>
    <section class="panel ${t.activeTab==="profile"?"":"hidden"}">${q()}</section>
    ${t.user?.role==="admin"?`<section class="panel ${t.activeTab==="control"?"":"hidden"}">${H()}</section>`:""}

    ${L()}
  </main>`},y=document.querySelector("#app"),S=async()=>{if(t.token)try{t.user=await l("/auth/me")}catch{t.user=null,t.token="",localStorage.removeItem("stories_token")}},v=()=>{y&&(y.innerHTML=x(),C(v),t.activeTab==="home"&&P(v),t.activeTab==="profile"&&R(v),t.activeTab==="control"&&t.user?.role==="admin"&&(T(),A()),t.authOpen&&!t.user&&E(v,S))};await S();v();
//# sourceMappingURL=index-CHT4X0Cm.js.map
