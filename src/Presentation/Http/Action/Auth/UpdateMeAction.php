...
use Stories\Application\Auth\UpdateProfile\UpdateProfileRequest;
...
final class UpdateMeAction extends JsonAction
{
    ...
    {
        try {
            $dto = UpdateProfileRequest::fromArray($this->body($request));
            $this->validator->validate($dto);

            return $this->respond(
                $request,
                $response,
                $this->handler->handle($this->auth->user($request)->id, $dto)->toArray(),
            );
